...
1. `docs/PROJECT_BRIEF.md` —— 细化目的、用户、MVP、严格 out-of-scope。
2. `docs/REQUIREMENTS_QUESTIONNAIRE.md` —— 见下面的"问卷规则"。
3. `docs/REQUIREMENTS_ANSWERS.md` —— 记录用户答案；用户跳过的题用显式默认值并标注。
4. `docs/PRD.md` —— 项目背景、目标、用户、MVP、in/out scope、用户故事、流程、
   页面、模块、数据、API、AI/LLM 流程、非功能需求、验收标准、风险、未来 roadmap。
...
6. `docs/ROADMAP.md` —— 8-20 个垂直切片任务；每个任务包含目标、scope、
   out-of-scope、文件、步骤、验收标准、验证命令、风险、回滚。
...
- **假设不阻塞。** 答案缺失时，写一个显式假设并继续。只有当未答的问题影响
  安全 / 成本 / 法律 / 不可逆架构时才停下。
- **Out of Scope 必须严格。** 不要让未来 roadmap 的功能漏进 MVP。把它们写到
  `Future Roadmap` 里。
